-- Programa el cron de war-results en Supabase (pg_cron + pg_net).
...
select cron.schedule(
  'war-results',              -- nombre del job (único)
  '5 9 * * 1',                 -- lunes 9:05 UTC (después de members-daily a las 9:00)
  $$
  select net.http_post(
    url     := 'https://vpdovzarpadcunhiisxw.supabase.co/functions/v1/war-results',
...
-- select * from cron.job_run_details where jobname = 'war-results' order by start_time desc limit 20;

-- Para eliminarlo si algo sale mal:
-- select cron.unschedule('war-results');
